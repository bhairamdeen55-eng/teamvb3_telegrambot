...
from utils.keyboards import admin_menu_kb, back_kb, confirmation_kb, yes_no_kb
from utils.texts import ERROR_TEXT
from db.models import User, Quiz, Subscription, UserRole, SubscriptionTier
...
async def confirm_broadcast(callback: CallbackQuery, state: FSMContext, session=None) -> None:
    ...
    for user in users[:100]:  # Limit to first 100 for now
        try:
            await callback.bot.send_message(user.id, broadcast_text)
            sent += 1
